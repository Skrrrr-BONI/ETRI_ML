...
parser = argparse.ArgumentParser(description='HR lifelog')
parser.add_argument('--cutoff', type=int, default=1)
args = vars(parser.parse_args())


def load_data():
    users = ['user01_06', 'user07_10', 'user11_12', 'user21_25', 'user26_30']
    X_train = None
    y_train = None
    for i, u in enumerate(users):
        with open('./data/train_{}.pkl'.format(u), 'rb') as f:
            train = pickle.load(f)
        if i == 0:
            X_train = train['X']
            y_train = train['y']
        else:
            X_train.extend(train['X'])
            y_train.extend(train['y'])
    with open('./data/valid.pkl', 'rb') as f:
        valid = pickle.load(f)
    return X_train, y_train, valid['X'], valid['y']
...
def feat_ext(X, Y):
    ...
    for x, y in tqdm(zip(X, Y)):
        ...
        y_list.append(y)
    return pd.concat(df_list, ignore_index=True), np.array(y_list)

X_train, y_train, X_valid, y_valid = load_data()
X_train_feat, y_train_feat = feat_ext(X_train, y_train)
X_val_feat, y_val_feat = feat_ext(X_valid, y_valid)
with open('./data/feat.pkl', 'wb') as f:
    pickle.dump({'train': [X_train_feat, y_train_feat], 'valid': [X_val_feat, y_val_feat]}, f)
